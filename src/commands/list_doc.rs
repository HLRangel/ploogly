...
fn get_frontmatter_block(origin: &[u8],
                            last: &mut usize, 
                            current: &mut usize
    ...
    if is_n_chars_before_n(origin, *current, b'-', 3, b'\n') {
        ...
        *last = *current;
        ...
        while !is_eof(origin, *current) {
            if origin[*current] == b'\n' {
                if is_n_chars_before_n(origin, *current + 1, b'-', 3, b'\n') {
                    *current += 4;
...
fn get_frontmatter_ctx(origin: &[u8], 
                        last: &mut usize, 
                        current: &mut usize
    ...
    return import_variables(&fmtxt);
}

fn docdata_get(path: &str, 
    ...
    let data: DocData = match cache.get(path) {
        ...
        _ => {
            ...
            let new_filename: String = format!("{}.html",
                hasher.finish());
...
fn doc_create_whole_ctx(locals: &HashMap<String, Vec<u8>>, 
    ...
    map.insert("path".to_string(), format!("posts/{}", filename).as_bytes().to_vec());
...
fn doc_place_ret_ctx(path: &str, 
    ...
    if !was_cached {
        ...
        let mut new_file: File = 
            File::create(format!("./out/site/posts/{}", &data.path))?;

        new_file.write_all(&produce(&to_produce, &mut map_for_template, cache, anon_stack)?)?;
    }

    return Ok(map_for_template);
}

fn ctxvec_to_sorted_ctxvec(vec: &Vec<HashMap<String, Vec<u8>>>,
                        var_to: &str
                    ) -> Vec<HashMap<String, Vec<u8>>> {
    let mut toret: Vec<HashMap<String, Vec<u8>>> = vec.clone();
    toret.sort_by(|a, b | {
        a.get(var_to).unwrap_or(&"0".as_bytes().to_vec())
...
fn doc(origin: &[u8], 
    ...
    if !is_eof(origin, *current) && 
        exists(&doc_path)? && exists(&template_path)? {
        let mut ctx: HashMap<String, Vec<u8>> = doc_place_ret_ctx(&doc_path, 
                                                            &template_path,
                                                                cache,
                                                                globals,
                                                            &mut Vec::new(), anon_stack)?;
...
fn iter_doc(origin: &[u8],
    ...
    for dire in read_dir(path)? {
        let ipath: String = String::from_str(dire?.path()
                    .to_str().unwrap()).unwrap();
        if metadata(&ipath)?.is_file() && ipath.ends_with(".md") {
            doc_place_ret_ctx(&ipath, template_path, cache, globals, &mut ctxlist, anon_stack)?;
        }
    }
...
fn docs_in(origin: &[u8], 
    ...
    if !is_eof(origin, *current) && 
        exists(&folder_path)? && metadata(&folder_path)?.is_dir() && exists(&template_path)? {

        return Ok(iter_doc(&inner, &folder_path, &template_path, &sortby, &order, cache, globals, anon_stack)?);
    }

    return Err(ErrorKind::InvalidInput.into());
}

pub fn list_doc(origin: &[u8],
            last: &mut usize, 
            current: &mut usize, 
            cache: &mut HashMap<String, DocData>, 
            globals: &HashMap<String, Vec<u8>>,

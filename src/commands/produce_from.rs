...
fn get_context(origin: &[u8],
                last: &mut usize,
                current: &mut usize
            ) -> Result<HashMap<String, Vec<u8>>, std::io::Error> {
    let arg: String = get_word(origin, last, current)?;

    let vars: HashMap<String, Vec<u8>> = 
        get_vars_from_file(&format!("./contexts/{arg}"))?;

    return Ok(vars);
}

fn iter_produce(template: Vec<u8>,
                path: &str,

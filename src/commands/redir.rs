...
pub fn redir(ctx: &mut CommandContext) -> Result<(), std::io::Error> {
    let mut to: PathBuf = path_as_relative(get_word_or_literal(ctx.origin, &mut ctx.last, &mut ctx.current)?);
    ...
    let varpath: String = String::from_utf8(match ctx.vars.get("path") {
        Some(var) => var.to_vec(),
